...
class Product < ApplicationRecord
  ...
  require 'product/product_attributes'
  ...
  # Example:
  #
  # Product.import("path/to/file.csv")
  def self.import(file)
    ...
    (2..spreadsheet.last_row).each do |i|
      ...
      if product = where(name: row['name']).take
        ...
        qty = row['qty'].to_i
        if qty > 0
          product.stock_level_adjustments.create!(description: t('shop.import'), adjustment: qty)
        end
      else
        product = new
  ...
  def self.open_spreadsheet(file)
    ...
    else fail t('shop.imports.errors.unknown_format', filename: File.original_filename)
    end
  end
