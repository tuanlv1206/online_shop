module ApplicationHelper
  ...
  # Helper method to display flash messages
  def display_flash
    html = ""
    flash.each do |severity, message|
      if message
        ...
        html += content_tag 'div', message, :id=> "flash-#{severity}"
      end
    end
    html
  end

  # Returns currency values with the currency unit as specified by the shop settings
  ...
  def number_to_weight(kg)
    "#{kg}#{t('shop.helpers.number_to_weight.kg', default: 'kg')}"
  end
  ...
  def navigation_manager_link(item)
    ...
    def status_tag(status)
      content_tag :span, t("shop.orders.statuses.#{status}"), class: "status-tag #{status}"
    end
    ...
    def settings_label(field)
      "<label for='settings_#{field}'>#{t("shop.settings.labels.#{field}")}</label>".html_safe
    end

    def settings_field(field, options = {})
      default = I18n.t('shop.settings.defaults')[field.to_sym]
      value = (params[:settings] && params[:settings][field]) || shop.settings[field.to_s]
      type = I18n.t('shop.settings.types')[field.to_sym] || 'string'
      case type
      when 'boolean'
        ''.tap do |s|
          ...
          s << label_tag("settings_#{field}_true", t("shop.settings.options.#{field}.affirmative", default: 'Yes'))
          s << radio_button_tag("settings[#{field}]", 'false', value == false, id: "settings_#{field}_false")
          s << label_tag("settings_#{field}_false", t("shop.settings.options.#{field}.negative", default: 'No'))
          s << '</div>'
        end.html_safe
      else
